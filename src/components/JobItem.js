...
export default class JobItem extends React.Component<Props> {
  ...
  renderDescription = (responsibility: ?string, requirement: ?string) => {
      ...
      <div className="sm-12 md-6 lg-4 col align-top" key={id}>
        <div className="card">
          ...
          <div className="card-body">
            ...
            <div className="margin">{this.renderDescription(responsibility, requirement)}</div>
            ...
            <div className="flex-center row">
              <Link to={`/job/${id}`} style={{ background: 'none' }}>
                <button className="paper-btn btn-secondary">К странице</button>
              </Link>
            </div>
          </div>
        </div>
